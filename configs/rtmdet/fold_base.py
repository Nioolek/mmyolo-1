_base_ = './rtmdet_x_comp_1280_yang6.py'

# 相比较yang5_semi_a40添加的内容：
# 1、半监督部分增加随机旋转（处理里加了print，待确认能不能打印）
...
strong_pipeline = [
    ...
    dict(type='RandomRotateYOLO'),
    dict(type='mmdet.PackDetInputs', meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape',
                                                'flip_state', 'matrix', 'scaleing_affine', 'rotate'))
]
...
unsup_pipeline = [
    ...
    dict(type='RandomCropYOLO', crop_size=img_scale, allow_negative_crop=True, show=True),   # (1600, 1600)
    dict(type='mmdet.Pad', size=img_scale, pad_val=dict(img=(114, 114, 114))),
    dict(
...
unlabeled_dataset = dict(
    ...
    metainfo=_base_.metainfo,
    data_root=data_root,
    ann_file='Semi-supervision/instances_new_test_pse.json',
...
# test_image = 'test2017/'
# test_image_info = 'annotations/test2.json'
# test_image = 'test2/'
...
#     outfile_prefix='test2-date0620_softnms065_185e',  # 要保存的 JSON 文件的前缀
# )
...
#     # metric='bbox',
#     format_only=True,  # 只将模型输出转换为coco的 JSON 格式并保存
#     outfile_prefix='val-date0620_220e_softnms065'
# )
